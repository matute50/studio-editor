...
const PROMPT_MAESTRO_GROK = `Hyper-photorealistic 8K exact identical portrait to reference image of Ara: zero deviation in facial bones, skin pores, peach fuzz, makeup, hair strands, eye reflections, clothing — perfect face lock.
Waist-up medium shot, standing in FIXED news studio background from reference (100% static, frozen, no movement, no changes, locked set).
Ara as confident Buenos Aires news anchor, direct eye contact to camera, delivering news in authentic porteño rioplatense Spanish accent.
She is actively saying right now: "{TEXTO_A_DECIR}"
EXTREME PRIORITY — DYNAMIC JAW AND MOUTH ENFORCEMENT: perfect lip synchronization with highly dynamic jaw movement, anatomically correct and visible jaw drop for open vowels and diphthongs in Argentine Spanish, strong mandibular articulation, noticeable jaw opening and closing transitions, no static jaw, no minimal or frozen mandible, active jaw motion even on consonants, dynamic mandible displacement matching phoneme energy (wide jaw drop on stressed 'a/e/o', subtle but visible on 'i/u', increased on aspirated /s/ finals and rehilado sounds).
Milimetric precise phoneme-to-viseme for porteño dialect: 
- strong dynamic jaw drop and forward protrusion for yeísmo rehilado /ʝ/ → [ʃ]/[ʒ] (sh/z sounds, e.g. "yo" → visible jaw shift + lip rounding/protrusion, "lluvia" → dynamic tongue + jaw),
- visible tongue elevation and forward/back movement for sibilants and retroflex approximants,
- relaxed jaw lowering and aspiration effects on final /s/ (e.g. "los" → jaw slightly drops with breathy release),
- full range of jaw positions: wide open for vowels like á-é-ó in stressed syllables, quick snaps for plosives (p/t/k), natural coarticulation without freezing,
- no plastic mouth, no immobile jaw, no subtle-only movement — enforce energetic, natural, anatomically realistic mandibular dynamics typical of fluent Buenos Aires Spanish speech, visible jaw muscle tension and release.
Subtle but realistic presenter gestures: micro head tilts, natural blinks, throat movement, professional posture with slight emphasis nods matching speech rhythm.
Ultra-cinematic realism: subsurface scattering, lip moisture/gloss, individual eyelashes/iris depth, broadcast 3-point lighting (key + fill + rim), shallow DoF sharp focus on face/mouth/jaw area, shot on Arri Alexa 65 or RED 8K 85mm f/1.4, no artifacts, no symmetry errors, maximum photorealism.
Vertical 9:16 social media format, 2K+ res, --ar 9:16 --v 6 --stylize 50 --q 2 --style raw --chaos 2`;
