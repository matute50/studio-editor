...
[IDENTIDAD_VISUAL:ARA_BUENOS_AIRES] Eye-level, locked-off medium shot of Ara, the professional news anchor from the provided reference image. Photorealistic facial identity lock, exact attire, and static background identical to the reference. [ANCLA_CONTEXTUAL_RIOPLATENSE] Una periodista de Buenos Aires, hablando con voz firme, decidida y autoridad profesional, usando voseo culto y cadencia del Cono Sur. [AUDIO_STUDIO_CONTROL] Character says: "Insert Guion con Fonética Forzada (ej: Mirá-áh, vos te-nés que sa-ber...)." [STRICT_PHONETICS] Read exactly as written. Execute "ssh" as /ʃ/ (sheísmo) for "LL" and "Y". Maintain "L" as a standard lateral alveolar. Use acute voseo stress (á-áh) to trigger the melodic "cantito". Eliminate all pauses between syllables unless a comma is present. Sound: complete studio silence. The anchor must stop speaking and close her mouth precisely when the audio ends. Negative prompt: neutral Spanish, Mexican accent, Spanish accent, italian accent, andaluz, chileno, colombiano, singing tone, lisp, ceceo, seseo peninsular, corporate motivational tone, stuttering, mumbling, hesitating, generic face, airbrushed skin, plastic texture, different ethnicity, altered eye color, changing facial identity, morphing, animated background logo, moving logo, logo animation, outdoor lighting, sunlight, rain, wind, wipe transitions, fade out, scene cuts, text overlays, subtitles, captions, title cards, typography, visual filters, post-processing effects, background music, sound effects, SFX, ambient noise, audio artifacts, extra dialogue, off-script talking, lip movement without audio, voice before speech, voice after speech, robotic speech, monotone delivery.`;
...
export function AvatarStudio() {
    ...
    const generarPromptParaClip = (clip: any) => {
        ...
        return PROMPT_MAESTRO_SYSTEM.replace(
            /Insert Guion con Fonética Forzada \(ej: Mirá-áh, vos te-nés que sa-ber\.\.\.\)\./,
            scriptProcesado
        );
    };
