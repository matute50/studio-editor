...
const PREDEFINED_CATEGORIES = [
    ...
    'HISTORIA',
    'FIERROS',
    'ARCHIVO SALADILLO VIVO',
    'ITEC ¨AUGUSTO CICARE¨ SALADILLO',
    'SEMBRANDO FUTURO'
];
...
export const YouTubeStudio: React.FC = () => {
    ...
    const fetchPublishedVideos = async () => {
        try {
            ...
            if (data) {
                ...
                // Initialize with predefined categories (starting at 0)
    ...
    return (
        <div className="h-[calc(100vh-8rem)] bg-slate-50 rounded-3xl p-8 border border-slate-200 animate-fadeIn flex flex-col gap-8 overflow-y-auto custom-scrollbar">
            {/* Header */}
                ...
                <div className="flex-1 space-y-4">
                    <div className="flex gap-2">
                        <input
                            ...
                            className="flex-1 px-6 py-4 rounded-2xl bg-slate-50 border border-slate-200 font-medium outline-none focus:ring-2 focus:ring-red-500 transition-all placeholder:text-slate-400 text-sm focus:bg-white"
                        />
                        <button
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3 max-h-[400px] overflow-y-auto custom-scrollbar p-1">
                        ...
                        <div key={video.id} className="bg-slate-50 p-3 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all group">
                            <div className="aspect-video bg-slate-200 rounded-xl overflow-hidden relative mb-3">
                                ...
                                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                                    <a href={video.url} target="_blank" rel="noreferrer" className="p-1.5 bg-white/20 hover:bg-white text-white hover:text-blue-600 rounded-full transition-all">
                                        <PlayCircle size={16} />
                                    </a>
                                    <button
                                        onClick={() => handleRename(video)}
                                        className="p-1.5 bg-white/20 hover:bg-yellow-400 text-white hover:text-yellow-900 rounded-full transition-all"
                                    ...
                                    <button
                                        ...
                                        className="p-1.5 bg-white/20 hover:bg-red-600 text-white rounded-full transition-all"
                                    ...
                                    <button
                                        ...
                                        className="p-1.5 bg-blue-600 hover:bg-blue-400 text-white rounded-full transition-all"
            ...
            <div className="flex-1">
                {loadingLib ? (
                    <div className="h-40 flex items-center justify-center text-slate-300"><Loader2 className="animate-spin w-8 h-8" /></div>
                ) : library.length === 0 ? (
                    <div className="h-40 flex flex-col items-center justify-center text-slate-300 opacity-50">
